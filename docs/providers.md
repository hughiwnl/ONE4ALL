...
- **Publish**: Content Publishing API. Creates a `REELS` media container from a **public URL** of the video (Repeat's signed media route), then `getStatus` polls the container (`IN_PROGRESS` → `FINISHED`), publishes it with `media_publish`, and reads the permalink.
- **Restrictions**: Business or Creator accounts linked to a Facebook Page only; MP4/MOV (H.264/AAC), 3 s – 15 min, ≤ 1 GB, width ≤ 1920; 100 API posts per 24 h; `APP_URL` must be publicly reachable over HTTPS (error `invalid_media` with an explanation otherwise); `instagram_content_publish` requires App Review outside app roles. Images/carousels and the Pages-less "Instagram Login" flow are not implemented yet.
...
Example: a hypothetical `tiktok` provider. Only the first two steps are platform-specific; the engine, database, queue and UI stay untouched.

### 1. Create the package

```
packages/providers/tiktok/
  package.json          name: @repeat/provider-tiktok, deps: @repeat/provider-sdk, @repeat/types, zod
  src/index.ts
  src/tiktok-publisher.ts
  src/tiktok-connector.ts
  src/errors.ts
  src/__tests__/tiktok.test.ts
```
...
export const tiktokSettingsSchema = z.object({
  ...
  privacy: z.enum(['PUBLIC_TO_EVERYONE', 'MUTUAL_FOLLOW_FRIENDS', 'SELF_ONLY']).default('PUBLIC_TO_EVERYONE'),
});
...
export class TikTokPublisher implements PublisherProvider<TikTokSettings> {
  ...
  readonly settingsFields = [
    ...
    { key: 'privacy', label: 'Privacy', type: 'select', default: 'PUBLIC_TO_EVERYONE', options: [...] },
  ];
  readonly notes = ['Unaudited TikTok apps can only post as private (SELF_ONLY).'];
...
export class TikTokConnector implements OAuthConnector {
  readonly id = 'tiktok'; // routes: /api/connect/tiktok/start|callback
  readonly label = 'TikTok';
  readonly platforms = ['tiktok'];
...
if (env.TIKTOK_CLIENT_KEY && env.TIKTOK_CLIENT_SECRET) {
  ...
  providers.register(new TikTokPublisher(connector));
} else {
  unconfigured.push({ platform: 'tiktok', displayName: 'TikTok', reason: 'Set TIKTOK_CLIENT_KEY and TIKTOK_CLIENT_SECRET' });
}
```
