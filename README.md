...
Upload a video, tick the accounts it should go to, press **Publish**. Repeat creates an independent publishing job per destination, uploads through each platform's official API, tracks every destination separately, and lets you retry just the ones that failed.
...
- Per-post destination selection, with platform-specific options (YouTube privacy/category, Instagram caption...).
...
- Official APIs only: YouTube Data API v3, Meta Graph API (Facebook Pages, Instagram professional accounts). No scraping, no private endpoints.
- Provider adapters are isolated behind a small interface so TikTok, X, LinkedIn, Threads, Bluesky, Pinterest... can be added without touching the publishing engine.
...
| **Instagram** | Videos as **Reels** (MP4/MOV, 3 s – 15 min, ≤ 1 GB)       | Instagram **Business or Creator** account linked to a Facebook Page             | Personal accounts are not supported by Meta's API. Instagram downloads the file from your server, so `APP_URL` must be a public HTTPS address. 100 API posts / 24 h. |
...
- **Instagram**: only **Business or Creator** accounts that are **linked to a Facebook Page** are discoverable and publishable. Personal accounts are not supported. Instagram fetches the video from a URL, so `APP_URL` must be public HTTPS (Repeat hands out time-limited signed URLs; nothing is publicly listable). Instagram enforces 100 API-published posts per account per 24 hours, MP4/MOV with H.264/AAC, 3 s – 15 min, ≤ 1 GB, width ≤ 1920 px. Videos are published as Reels (Meta retired the plain `VIDEO` type).
...
- Facebook Reels and Stories; Instagram image posts and carousels; Instagram Login (Pages-less) flow
- TikTok, X, LinkedIn, Threads, Bluesky, Pinterest providers
